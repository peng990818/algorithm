...
// 
//
// Related Topics 广度优先搜索 哈希表 字符串 👍 1485 👎 0


//leetcode submit region begin(Prohibit modification and deletion)
func ladderLength(beginWord string, endWord string, wordList []string) int {
// 广搜
mp := make(map[string]struct{}, len(wordList))
for i:=0;i<len(wordList);i++ {
mp[wordList[i]] = struct{}{}
}
if _, ok := mp[endWord];!ok {
return 0
}

// key 当前单词 value 当前路径长度
...
_, ok1 := mp[string(newWord)]
_, ok2 := visitMap[string(newWord)]
if ok1 && !ok2 {
visitMap[string(newWord)] = path+1
